  ...
  try {
    ...
    if ((match = data.match(/^pay_accept_(order_\d{13}_[a-z0-9]{6,8})$/))) {
      ...
      try {
        ...
        // Categorize errors — show specific info without exposing raw internals
        let userMsg: string;
        ...
        } else if (errMsg.includes('timeout') || errMsg.includes('AbortError') || errMsg.includes('ETIMEDOUT') || errMsg.includes('ECONNRESET')) {
